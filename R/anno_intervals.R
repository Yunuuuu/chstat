...
#' @param width A [unit] object specifying the width of the tip head.
#' @inheritParams grid::grid.points 
#' @param point_gp A [gpar] objects define the point attributes.
#' @param line_gp A [gpar] objects define the line attributes.
...
anno_errorbar <- function(x, which = NULL, width = 0.9, pch = 19, size = 1, point_gp = gpar(), line_gp = gpar(), band = TRUE, band_col = c("white", "#eff3f2")) {
    ...
    ComplexHeatmap::AnnotationFunction(
        ...
        var_import = list(
            ...
            point_gp = point_gp, line_gp = line_gp,
            pch = pch, size = size, width = width,
            band = band, band_col = band_col
        ),
...
utils::globalVariables(c("limits"))
